...
export const Login = () => {
    ...
    const { login: authLogin } = useAuth(); // desde el AuthContext

    const handleLogin = async (e: React.FormEvent) => {
        e.preventDefault();
    
        try {
            const response = await login({ correo, contrasena: contrasena });
            authLogin(response.token); // guarda el token y decodifica
    
            const decoded = JSON.parse(atob(response.token.split('.')[1]));
            const role = decoded["http://schemas.microsoft.com/ws/2008/06/identity/claims/role"];
    
            if (role === 'Usuario') {
                navigate('/solicitud'); // ruta inicial del usuario
            } else if (role === 'Admin') {
                navigate('/admin'); // si tuvieras una vista admin
            } else {
                navigate('/'); // fallback o error
            }
        } catch (error) {
